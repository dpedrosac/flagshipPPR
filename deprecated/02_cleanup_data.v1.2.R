...
#
# Version 1.2 # 2025-26-11 # Changes in structure and added further questionnaires
...
## PDQ39
pdq39_raw <- read.csv2(
  file     = file.path(base_dir, "raw_data", "pdq39_sortiert_Final.csv"),
  encoding = "UTF-8"
)
...
tab_city <- CreateTableOne(
  vars = c(
    "sex_at_birth","housing",
    "birth_country","marital_status","native_language","education",
    "currently_employed", "employment_status",
    "clinical_diagnosis_ips", "hoehnyahrlevel", "Teil1", "Teil2", "Teil3", "Teil4", "total_score_moca", "bdi_score", "nmss_total"
  ),
  strata = "group.y",
  ...
  factorVars = c(
    ...
    "birth_country","marital_status","native_language","education",
    "currently_employed", "employment_status", 
    "clinical_diagnosis_ips", "hoehnyahrlevel"
  )
)
print(
  tab_city,
...
write.csv(df_tab_city, file.path(results_dir, "demographics_tableone.csv"), row.names = FALSE)
...
if (isTRUE(sanity_check)) {
  ...
  id_col        <- intersect(id_candidates, names(pdq39_raw))[1]  # may be NA if none
...
score_pct <- function(df, cols, item_max = 2) {
...
pdq39_raw <- pdq39_raw %>%
  rowwise() %>%
  mutate(
    pdq39_sum_index = sum(c_across(61:68), na.rm = TRUE) / 8
  ) %>%
  ungroup()
...
pdq39_raw <- pdq39_raw %>%
  ...
  mutate(
    pdq39_sum_index =
      sum(c_across(61:68), na.rm = TRUE) /
      sum(!is.na(c_across(61:68)))
  ) %>%
  ungroup()
